export class AgentSheet extends ActorSheet {
  ...
  /** @override */
    ...
    async _onRoll(event) {
        ...
        //r.roll().toMessage({speaker: speaker, flavor: action});
        await r.evaluate({async: true});
        let rolls = (r.terms)[0].results;
        let roll_status = this.getActionRollStatus(rolls, false);
        let result = await renderTemplate("systems/buried-secrets/templates/secrets-roll.html", {rolls: rolls, roll_status: roll_status, zeromode: false});
        let messageData = {
            speaker: speaker,
            content: result,
